...
<link rel="stylesheet" href="http://localhost/fyproj/public/css/sadd_prog_css.css">
...
<div class="intro-content">
	<div class="registration">	
		...
		<form id="formRegister" action ="<?php echo base_url()?>/sedit_prog/update/<?php echo $stf->StfID ?>/<?php echo $prg->PrgID ?>/<?php echo $prg->FacID?>" method ="post" enctype='multipart/form-data'>
		<?php echo $this->session->flashdata('status'); ?>
		<table id="prog-tb">
		
		<tr><td>Programme Code: </td>
			<td><input type="text" name= "code" value="<?php echo $prg->PrgID ?>" placeholder="HC00" size="10"/>
		</td></tr>
		<tr><td><br/></td></tr>
		<tr><td>Programme Name: </td>
			<td><input type="text" name= "name" value="<?php echo $prg->PrgName ?>" placeholder="Computer Science" size="50"/>
		</td></tr>
		<tr><td><br/></td></tr>
		<tr><td>Picture: </td>
		...
		<tr><td colspan="2"><textarea row="2" cols="66" name= "desc" placeholder="Description"size="70"/><?php echo $prg->PrgDesc ?></textarea></td></tr>
		<tr><td><br/></td></tr>
		<tr><td>Career Prospect: </td>
			<td><input type="text" name= "career" value="<?php echo $prg->PrgProspect ?>" placeholder="Software Developer" size="50"/>
		</td></tr>
		<tr><td><br/></td></tr>
		<tr><td colspan="2">Programme Objectives</td></tr>
		<?php $objn=0 ?>
		<?php foreach($OBJECTIVE as $obj){?>
		<tr><td colspan="2" class="c"><input type="text" name="obj[<?php echo $objn ?>]" value="<?php echo $obj->ObjDesc ?>" placeholder="objective" size="66"/></td>
			<td class="del-cco"><button type="button" onclick="delObj(this)" i="button" i class="fa fa-minus"></i></button></td></tr>
		<?php $objn+=1 ?>
		<script>obj_num+=1;</script>
		<?php }?> 
		...
		<tr><td hidden><input type="text" name="ploid[<?php echo $plon ?>]" value="<?php echo $plo->PLOID ?>"/></td>
			<td><input type="text" class="plo" name="plo[<?php echo $plon ?>]" value="<?php echo $plo->PLONum ?>" placeholder="PLO1" size="10"/></td>
			<td><textarea row="2" cols="50" name= "descrp[<?php echo $plon ?>]" placeholder="Description"size="70"/><?php echo $plo->PLODesc ?></textarea></td>
			<td class="del-cco"><button type="button" onclick="delPLO(this)" i="button" i class="fa fa-minus"></i></button></td></tr>
		<?php $plon+=1 ?>
		<script>plo_num+=1;</script>
		<?php }?> 
		...
		<tr><td>Duration (years): </td>
			<td><input type="text" name= "duration" value="<?php echo $prg->PrgDuration ?>" placeholder="4" size="5"/>
		</td></tr>
		<tr><td><br/></td></tr>
		<tr><td>University Core</td><td style="float:right; padding-right:30px;">Semester</td></tr>
		<?php $unin=0 ?>
		<?php foreach($UNIC as $unic){?>
		<tr><td colspan="2" class="ctg-drop">
			<select name="uni[<?php echo $unin ?>]">
			<option style="display:none" value="<?php echo $unic->CrsID?>"><?php echo $unic->CrsName?></option>
			<?php foreach($AUNI as $uni){?>
				<option value="<?php echo $uni->CrsID?>"><?php echo $uni->CrsName?></option>
			<?php }?>
			</select>
			<input type="text" name="usem[<?php echo $unin ?>]" value="<?php echo $unic->Semester?>" placeholder="1" size="7"/></td>
			<td class="del-cco"><button type="button" onclick="delUni(this)" i="button" i class="fa fa-minus"></i></button></td></tr>
		<?php $unin+=1 ?>
		<script>uni_num+=1;</script>
		<?php }?> 
		...
		<?php $facn=0 ?>
		<?php foreach($FACC as $facc){?>
		<tr><td colspan="2" class="ctg-drop">
			<select name="fac[<?php echo $facn ?>]">
			<option style="display:none" value="<?php echo $facc->CrsID?>"><?php echo $facc->CrsName?></option>
			<?php foreach($AFAC as $fac){?>
				<option value="<?php echo $fac->CrsID?>"><?php echo $fac->CrsName?></option>
			<?php }?>
			</select>
			<input type="text" name="fsem[<?php echo $facn ?>]" value="<?php echo $facc->Semester?>" placeholder="1" size="7"/></td>
			<td class="del-cco"><button type="button" onclick="delFac(this)" i="button" i class="fa fa-minus"></i></button></td></tr>
		<?php $facn+=1 ?>
		<script>fac_num+=1;</script>
		<?php }?> 
		...
		<?php $prgn=0 ?>
		<?php foreach($PRGC as $prgc){?>
		<tr><td colspan="2" class="ctg-drop">
			<select name="prg[<?php echo $prgn ?>]">
			<option style="display:none" value="<?php echo $prgc->CrsID?>"><?php echo $prgc->CrsName?></option>
			<?php foreach($APRG as $prg){?>
				<option value="<?php echo $prg->CrsID?>"><?php echo $prg->CrsName?></option>
			<?php }?>
			</select>
			<input type="text" name="psem[<?php echo $prgn ?>]" value="<?php echo $prgc->Semester?>" placeholder="1" size="7"/></td>
			<td class="del-cco"><button type="button" onclick="delPrg(this)" i="button" i class="fa fa-minus"></i></button></td></tr>
		<?php $prgn+=1 ?>
		<script>prg_num+=1;</script>
		<?php }?> 
		...
		<tr><td colspan="2" class="ctg-drop">
			<select name="ele[<?php echo $elen ?>]">
			<option style="display:none" value="<?php echo $elec->CrsID?>"><?php echo $elec->CrsName?></option>
			<?php foreach($AELE as $ele){?>
				<option value="<?php echo $ele->CrsID?>"><?php echo $ele->CrsName?></option>
			<?php }?>
			</select>
			<input type="text" name="esem[<?php echo $elen ?>]" value="<?php echo $elec->Semester?>" placeholder="1" size="7"/></td>
			<td class="del-cco"><button type="button" onclick="delEle(this)" i="button" i class="fa fa-minus"></i></button></td></tr>
		<?php $elen+=1 ?>
		<script>ele_num+=1;</script>
		<?php }?> 
